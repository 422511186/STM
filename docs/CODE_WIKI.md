...
```
SSH-Tunnel-Manager/
├── main.py                    # 项目入口文件（支持 GUI/CLI/Daemon）
├── stm_cli.py                 # CLI 专用入口（用于打包 stm.exe）
...
### 1.1 CLI 专用入口 (stm_cli.py)

**文件位置**: `stm_cli.py`

**功能**: CLI 专用入口，用于打包单文件 `stm.exe`。

**运行方式**:

```bash
# 源码运行
python stm_cli.py status
python stm_cli.py list
...
#### 构建脚本 (build_with_cli.bat)

使用 PyInstaller 将 CLI 打包为单文件 `stm.exe`。
...
|------|------|
| `--onefile` | 单文件输出 |
| `--console` | 控制台应用（CLI 模式） |
| 包含所有依赖 | Python 运行时、第三方库、DLL |
| 跨机器运行 | 无需安装 Python 或依赖 |
...
# 添加隧道
stm add mysql_tunnel 10.0.0.10 ubuntu 13306 3306 --pkey ~/.ssh/id_rsa

# 启动隧道
stm start mysql_tunnel
