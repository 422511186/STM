...
export default function AddTunnelDialog({ isOpen, onClose, onSuccess }: AddTunnelDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to add tunnel:', err)
      setSubmitError(err.response?.data?.message || '添加隧道失败')
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-card dark:bg-bg-card-dark rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 border-b border-border-light dark:border-border-light-dark">
          <h2 className="text-lg font-semibold text-text dark:text-text-dark">添加隧道</h2>
        </div>
