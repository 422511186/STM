...
# 禁用代理，确保本地 Daemon 请求不走代理
NO_PROXY_LOCAL = {"http": None, "https": None}

def notify_daemon_reload():
    try:
        requests.post(f"{DAEMON_URL}/config/reload", timeout=2, proxies=NO_PROXY_LOCAL)
...
def daemon(action: str = typer.Argument(..., help="start 或 stop")):
    ...
    if action == "start":
        ...
        if sys.platform == "win32":
            ...
            # WSL: 使用 nohup 配合 subprocess 确保守护进程正确分离
            subprocess.Popen(
                ["nohup", sys.executable, "-m", "daemon.server"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                stdin=subprocess.DEVNULL,
                env=env,
        ...
        try:
            requests.post(f"{DAEMON_URL}/shutdown", timeout=2, proxies=NO_PROXY_LOCAL)
...
def start(names: List[str] = typer.Argument(..., help="要启动的隧道名称，支持多个")):
    ...
    try:
        for name in names:
            r = requests.post(f"{DAEMON_URL}/tunnels/{name}/start", proxies=NO_PROXY_LOCAL)
...
def stop(name: str):
    """停止指定隧道"""
    try:
        r = requests.post(f"{DAEMON_URL}/tunnels/{name}/stop", proxies=NO_PROXY_LOCAL)
        typer.echo(r.json()["message"])
    except requests.exceptions.ConnectionError:
...
def status():
    """列出所有隧道及其状态"""
    try:
        r = requests.get(f"{DAEMON_URL}/tunnels", proxies=NO_PROXY_LOCAL)
        data = r.json()
        if not data:
            typer.echo("暂无隧道配置。")
            return
